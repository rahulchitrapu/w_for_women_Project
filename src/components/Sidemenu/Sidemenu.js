        ...
        <div className="sidemenu">
            <div>
                ...
                {appliedFilters.length>0 && <div className='appliedFiltes'>{appliedFilters.map((ele)=>{
                    return(<div key={ele} className='element-container'>{appliedFilters.length>1&&<span className='cancel_sidemenu' onClick={()=>cancelfunc(ele)}>X</span>}<span className='element'>{`${ele}`}</span></div>)
